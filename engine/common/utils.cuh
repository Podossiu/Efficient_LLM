...
// __global__ : host에서 호출 (Dynamic parallelism으로 디바이스에서 호출 가능), device에서 실행 
// __device__ : device에서 호출, device에서 실행 
// float2 : float 2개 들어있는 변수
...
// warp shuffle: shared memory를 사용하지 않고 워프 내의 스레드간의 변수 교환 
// lane ID 비트별 xor을 기반으로 레인에서 복사 
template<typename T>
